			...
			<TableBody>
				{apps.map((app) => (
					<TableRow key={app.id}>
						...
						<TableCell>
							<Badge variant={app.type === 'paid' ? 'default' : 'secondary'}>{app.type}</Badge>
						</TableCell>
		...
		<div className='container mx-auto p-4'>
			...
			<Tabs defaultValue='All' className='w-full'>
				...
				<TabsContent value='All'>
					{currentApplications.length > 0 ? renderTable(currentApplications, true) : <p>No applications found.</p>}
				</TabsContent>
				<TabsContent value='Applied'>
					{currentApplications.filter((app) => app.status === 'Applied').length > 0 ? (
						renderTable(currentApplications.filter((app) => app.status === 'Applied'))
					) : (
						<p>No applied applications found.</p>
					)}
				</TabsContent>
				<TabsContent value='Saved'>
					{currentApplications.filter((app) => app.status === 'Saved').length > 0 ? (
						renderTable(currentApplications.filter((app) => app.status === 'Saved'))
					) : (
						<p>No saved applications found.</p>
					)}
				</TabsContent>
				<TabsContent value='Booked'>
					{currentApplications.filter((app) => app.status === 'Booked').length > 0 ? (
						renderTable(currentApplications.filter((app) => app.status === 'Booked'))
					) : (
						<p>No booked applications found.</p>
					)}
				</TabsContent>
				<TabsContent value='Unsuccessful'>
					{currentApplications.filter((app) => app.status === 'Unsuccessful').length > 0 ? (
						renderTable(currentApplications.filter((app) => app.status === 'Unsuccessful'))
					) : (
						<p>No unsuccessful applications found.</p>
					)}
				</TabsContent>
				<TabsContent value='Invited'>
					{currentApplications.filter((app) => app.status === 'Invited').length > 0 ? (
						renderTable(currentApplications.filter((app) => app.status === 'Invited'))
					) : (
