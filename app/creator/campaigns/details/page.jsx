...
import { FaFacebookF, FaInstagram, FaTiktok, FaYoutube, FaLinkedinIn, FaSnapchatGhost } from 'react-icons/fa';
...
const campaignData = {
	...
	contentNeeded: {
		type: 'Reel',
		description: 'Show us how using the Bearable Twig, will make your most unbearable moments in life, bearable!',
	},
		...
		<div className='container mx-auto p-4 max-w-4xl'>
			...
			<Card>
				<CardHeader className='relative p-0'>
					<img src={campaignData.image} alt={campaignData.title} className='w-full h-64 object-cover rounded-t-lg' />
				...
				<CardContent className='p-6'>
					<div className='flex justify-between items-start mb-4'>
						<div>
							<CardTitle className='text-2xl mb-2'>{campaignData.title}</CardTitle>
							<CardDescription className='text-lg mb-2'>{campaignData.brand}</CardDescription>
							<p className='text-sm font-medium'>
								Product valued at {campaignData.currency} {campaignData.productValue.toFixed(2)}
							</p>
						</div>
					...
					<div className='grid grid-cols-2 gap-4 mb-4'>
						<div>
							...
							<Link href={campaignData.website} className='text-blue-600 hover:underline block mt-1'>
								{campaignData.website}
							</Link>
							<div className='flex space-x-4 mt-2'>
								<Link href={campaignData.instagram || '#'} className='text-gray-400 hover:text-gray-800'>
									<FaInstagram className='w-7 h-7' />
								</Link>
								<Link href={campaignData.facebook || '#'} className='text-gray-400 hover:text-gray-800'>
									<FaFacebookF className='w-7 h-7' />
								</Link>
								<Link href={campaignData.tiktok || '#'} className='text-gray-400 hover:text-gray-800'>
									<FaTiktok className='w-7 h-7' />
								</Link>
								<Link href={campaignData.youtube || '#'} className='text-gray-400 hover:text-gray-800'>
									<FaYoutube className='w-7 h-7' />
								</Link>
								<Link href={campaignData.linkedin || '#'} className='text-gray-400 hover:text-gray-800'>
									<FaLinkedinIn className='w-7 h-7' />
								</Link>
								<Link href={campaignData.snapchat || '#'} className='text-gray-400 hover:text-gray-800'>
						...
						<Table>
							<TableBody>
								...
								<TableRow>
									...
									<TableCell>{campaignData.idealCreator.platforms.join(', ')}</TableCell>
								</TableRow>
								<TableRow>
									<TableCell className='font-medium'>Categories</TableCell>
									<TableCell>
										<div className='flex flex-wrap gap-2'>
											{campaignData.idealCreator.categories.map((category, index) => (
					...
					<div className='mb-4'>
						...
						<ul className='list-disc pl-5'>
							{campaignData.creatorResponsibilities.map((responsibility, index) => (
				...
				<CardFooter className='flex justify-between items-center'>
					<Button variant='outline' onClick={handleSaveCampaign} className='flex items-center'>
