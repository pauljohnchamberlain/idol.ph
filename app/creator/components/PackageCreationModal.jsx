...
export default function PackageCreationModal() {
	...
	const [usageRights, setUsageRights] = useState({ social: false, online: false, offline: false });
	...
	const handleOtherInclusionChange = (index, value) => {
		setOtherInclusions((prev) => prev.map((item, i) => (i === index ? value : item)));
	};
	...
	return (
		<Card className='w-full mx-auto max-h-[80vh] overflow-y-auto'>
			...
			<CardContent>
				{step === 1 && (
					<div>
						<div className='space-y-6'>
							<div>
								<h2 className='text-3xl font-bold mb-4'>Describe your package</h2>
								...
								<Textarea
									...
									className='mt-1'
								/>
								<div className='text-right text-sm text-muted-foreground'>{packageDescription.length}/200</div>
							</div>
							...
							<div>
								...
								<div className='bg-blue-50 border border-blue-200 rounded-md p-4 flex items-start space-x-3'>
									...
									<div>
										<h4 className='font-semibold text-blue-700'>Keep it short and simple</h4>
										<p className='text-sm text-blue-600'>
											By keeping your package simple and targeted to one platform, you can refine your message and cater
											to specific brands. Start strong with a focused package, then impress the brand with your
											expertise.
										</p>
									</div>
								</div>
								...
								{[
									...
									<div
										...
										className={`p-4 border rounded-md cursor-pointer ${
											selectedPlatform === name ? 'bg-blue-50 border-blue-500' : 'bg-white'
										}`}
						...
						<div className='mt-4'>
							...
							<p className='text-sm text-muted-foreground mt-2'>
								You can specify detailed content requirements once the campaign has been submitted
							</p>
						</div>
						...
						<div>
							...
							<div className='grid grid-cols-2 gap-4'>
								<div
									className={`p-4 border rounded-md cursor-pointer ${
										contentUsage === 'with-rights' ? 'bg-blue-50 border-blue-500' : 'bg-white'
									}`}
									onClick={() => setContentUsage('with-rights')}
								>
									<h4 className='font-semibold'>With usage rights</h4>
									<p className='text-sm text-gray-600'>Content will be used by the brand</p>
								</div>
								<div
									className={`p-4 border rounded-md cursor-pointer ${
										contentUsage === 'no-rights' ? 'bg-blue-50 border-blue-500' : 'bg-white'
									}`}
									onClick={() => setContentUsage('no-rights')}
								>
									<h4 className='font-semibold'>Brand will not have usage rights</h4>
									<p className='text-sm text-gray-600'>You will have usage rights</p>
								</div>
							</div>
							{contentUsage === 'with-rights' && (
								<div className='mt-4 space-y-4'>
									...
									<div>
										...
										<p className='text-sm text-gray-600 mb-2'>You can select multiple</p>
										<div className='space-y-2'>
											<div className='flex items-center space-x-2'>
												<Checkbox
													id='social'
													checked={usageRights.social}
													onCheckedChange={(checked) => setUsageRights((prev) => ({ ...prev, social: checked }))}
												/>
												<Label htmlFor='social'>Social</Label>
											</div>
											<p className='text-sm text-gray-600 ml-6'>
												This includes but is not limited to Instagram, TikTok etc...
											</p>
											<div className='flex items-center space-x-2'>
												<Checkbox
													id='online'
													checked={usageRights.online}
													onCheckedChange={(checked) => setUsageRights((prev) => ({ ...prev, online: checked }))}
												/>
												<Label htmlFor='online'>Online</Label>
											</div>
											<p className='text-sm text-gray-600 ml-6'>This could be on websites etc...</p>
											<div className='flex items-center space-x-2'>
												<Checkbox
													id='offline'
													checked={usageRights.offline}
													onCheckedChange={(checked) => setUsageRights((prev) => ({ ...prev, offline: checked }))}
												/>
												<Label htmlFor='offline'>Offline</Label>
											</div>
											<p className='text-sm text-gray-600 ml-6'>
												This includes but is not limited to billboards, train ads etc...
											</p>
										</div>
									</div>
									<div>
										<Label htmlFor='usage-details'>Usage details (optional)</Label>
										<Textarea id='usage-details' placeholder='Enter usage details' />
										<div className='text-right text-sm text-gray-600'>0/250</div>
									</div>
								</div>
							)}
						</div>
						...
						<div>
							...
							<p className='text-sm text-muted-foreground mb-2'>
								Boosting includes any paid amplification of a creator's post by a brand
							</p>
							<div className='grid grid-cols-2 gap-4'>
								<div
									className={`p-4 border rounded-md cursor-pointer ${
										boostingOption === 'offer-boosting' ? 'bg-blue-50 border-blue-500' : 'bg-white'
									}`}
									onClick={() => setBoostingOption('offer-boosting')}
								>
									<h4 className='font-semibold'>Offer boosting option</h4>
									<p className='text-sm text-gray-600'>Client can boost content</p>
								</div>
								<div
									className={`p-4 border rounded-md cursor-pointer ${
										boostingOption === 'no-boosting' ? 'bg-blue-50 border-blue-500' : 'bg-white'
									}`}
									onClick={() => setBoostingOption('no-boosting')}
								>
									<h4 className='font-semibold'>Don't offer boosting option</h4>
									<p className='text-sm text-gray-600'>Client cannot boost content</p>
								</div>
							</div>
							{boostingOption === 'offer-boosting' && (
								<div className='mt-4 space-y-4'>
									...
									<div>
										<Label htmlFor='boosting-details'>Boosting details (optional)</Label>
										<Textarea
											id='boosting-details'
				...
				{step === 2 && (
					<div>
						<p className='mb-4'>You can upload up to three contents. Minimum 1 required. ({uploads.length}/3)</p>
						<div className='grid grid-cols-2 gap-4'>
				...
				{step === 3 && (
					<div className='space-y-6'>
						<div>
							<h3 className='text-lg font-semibold mb-2'>Price and Payment</h3>
						...
						<div>
							<h3 className='text-lg font-semibold mb-2'>Delivery and Revisions</h3>
							<div className='space-y-4'>
								<div>
									<Label htmlFor='delivery-days'>How many days can you deliver the content?</Label>
						...
						<div>
							...
							<p className='text-sm text-muted-foreground mb-2'>
								Add services that are not defined in the deliverables such as pre production processes (script writing
								etc.)
							</p>
							{otherInclusions.map((inclusion, index) => (
								<div key={index} className='mb-2'>
									<Input
										value={inclusion}
										onChange={(e) => handleOtherInclusionChange(index, e.target.value)}
										placeholder='Add inclusion here'
									/>
									<p className='text-xs text-right text-muted-foreground'>{inclusion.length}/30</p>
								</div>
							))}
						</div>
					</div>
			...
			<CardFooter className='flex justify-between'>
				<Button variant='outline' onClick={() => setStep((prev) => Math.max(prev - 1, 1))}>
